{
module Language.Bash.Parse.Happy where

import Language.Bash.Parse.Alex
...
%token
    ...
    SQUOTE       { TokenSingleQuote }
    DQUOTE       { TokenDoubleQuote }
    ESCAPEDCHAR  { TokenEscapedChar $$ }
    CHAR         { TokenChar $$ }

%%

WORD:  SQUOTE SOMEWORD_Q(SOMECHAR) SQUOTE     { [ Single $2 ] }
    |  DQUOTE SOMEWORD_Q(SOMECHAR_NL) DQUOTE  { [ Double $2 ] }
    |  SOMEWORD                               { $1 }

SOMEWORD:  SOMECHAR_NL           { $1 }
        |  SOMECHAR_NL WORDLIKE  { $1 <> $2 }
        |  SOMECHAR_NL SOMEWORD  { $1 <> $2 }
        |  WORDLIKE SOMEWORD     { $1 <> $2 }

SOMEWORD_Q(c):  c                       { $1 }
             |  c SOMEWORD_Q(c)         { $1 <> $2 }
             |  WORDLIKE                { $1 }
             |  WORDLIKE SOMEWORD_Q(c)  { $1 <> $2 }

SOMECHAR:  ESCAPEDCHAR  { [ Escape $1 ] }
        |  CHAR         { [ Char $1 ] }

SOMECHAR_NL:  ESCAPEDCHAR  { if $1 == '\n' then [] else [ Escape $1 ] }
           |  CHAR         { [ Char $1 ] }

WORDLIKE:  IF        { stringToWord "if" }
        |  THEN      { stringToWord "then" }
        |  ELSE      { stringToWord "else" }
        |  ELIF      { stringToWord "elif" }
        |  FI        { stringToWord "fi" }
        |  CASE      { stringToWord "case" }
        |  ESAC      { stringToWord "esac" }
        |  FOR       { stringToWord "for" }
        |  SELECT    { stringToWord "select" }
        |  WHILE     { stringToWord "while" }
        |  UNTIL     { stringToWord "until" }
        |  DO        { stringToWord "do" }
        |  DONE      { stringToWord "done" }
        |  IN        { stringToWord "in" }
        |  FUNCTION  { stringToWord "function" }
        |  TIME      { stringToWord "time" }
        |  COPROC    { stringToWord "coproc" }
