...
happyExpList = Happy_Data_Array.listArray (0,20) ([4096,2048,0,0,0,508,0,0,0,0,0,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_wordParse","word","sword","schars","schar","COMMENT","ESC","DOLLAR","DQUOTE","META","SQUOTE","WORD","%eof"]
        bit_start = st * 15
        bit_end = (st + 1) * 15
...
happyReduction_9 (HappyTerminal (TokenMetaChar happy_var_1))
	 =  HappyAbsSyn7
		 (WordChar happy_var_1
	)
happyReduction_9 _  = notHappyAtAll 

happyReduce_10 = happySpecReduce_1  7 happyReduction_10
happyReduction_10 (HappyTerminal (TokenWordChar happy_var_1))
	 =  HappyAbsSyn7
		 (WordChar happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenDollar -> cont 10;
	TokenDouble -> cont 11;
	TokenMetaChar happy_dollar_dollar -> cont 12;
	TokenSingle -> cont 13;
	TokenWordChar happy_dollar_dollar -> cont 14;
	_ -> happyError' ((tk:tks), [])
	}
...
wordParse tks = happyRunIdentity happySomeParser where
