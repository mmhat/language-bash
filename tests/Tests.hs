...
unittests = testGroup "Unit tests"
  ...
  , tp "cat <<EOF\nasd\\`\nEOF" $ wrapCommand
        (Command ()
            ...
            [Heredoc () (Here ()) "EOF" False
                [Char 'a', Char 's', Char 'd', Escape '`', Char '\n']])
  , tp "cat <<EOF\nfoo\nEOF\n  cat" $ wrapCommands
        [ Command ()
            (SimpleCommand () [] [stringToWord "cat"])
            [Heredoc () (Here ()) "EOF" False (stringToWord "foo\n")]
        , Command () (SimpleCommand () [] [stringToWord "cat"]) []
        ]
  , tp "cat <<\"EOF\"\nasd\\`\nEOF" $ wrapCommand
        (Command ()
            (SimpleCommand () [] [stringToWord "cat"])
            [Heredoc () (Here ()) "EOF" True
                [Char 'a', Char 's', Char 'd', Char '\\', Char '`', Char '\n']])
  , tp "cat <<EOF1 <<EOF2\nt1\nEOF1\nt2\nEOF2" $ wrapCommand
        (Command ()
            (SimpleCommand () [] [stringToWord "cat"])
            [ Heredoc () (Here ()) "EOF1" False (stringToWord "t1\n")
            , Heredoc () (Here ()) "EOF2" False (stringToWord "t2\n")
            ])
  ...
  , tp "function foo() { true; }" $ wrapCommand
        (Command ()
            (FunctionDef () "foo" $ wrapCommand
                (Command () (SimpleCommand () [] [stringToWord "true"]) [])
            ) [])
  , tp "!awesome-function-name-2.0+() { true; }" $ wrapCommand
        (Command ()
            (FunctionDef () "!awesome-function-name-2.0+" $ wrapCommand
                (Command () (SimpleCommand () [] [stringToWord "true"]) [])
            ) [])
  , tp "cat <<EOF\nasd\nEOF\ntrue" $ wrapCommands
        [ Command ()
            (SimpleCommand () [] [stringToWord "cat"])
            [Heredoc () (Here ()) "EOF" False (stringToWord "asd\n")]
        , Command () (SimpleCommand () [] [stringToWord "true"]) []
        ]
  , tp "true\r\nfalse" $ wrapCommands
        [ Command () (SimpleCommand () [] [stringToWord "true"]) []
  ...
  , tp "for function in foo; do true; done" $ wrapCommand
        (Command ()
            (For () "function" (WordList () [stringToWord "foo"]) $ wrapCommand
                (Command () (SimpleCommand () [] [stringToWord "true"]) [])
            ) [])
  , tp "<<EOF\ncomment\nEOF" $ wrapCommand
        (Command ()
            (SimpleCommand () [] [])
            [Heredoc () (Here ()) "EOF" False (stringToWord "comment\n")])
  ]
